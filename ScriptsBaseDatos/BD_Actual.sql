...
CREATE TABLE public.NoticiaSentimiento (
  IdNoticia integer GENERATED ALWAYS AS IDENTITY NOT NULL,
  IdEmpresa integer NOT NULL,
  FechaPublicacion timestamp without time zone DEFAULT now(),
  Titular character varying NOT NULL,
  Contenido text,
  Sentimiento numeric,
  Etiqueta character varying,
  UrlFuente character varying,
  CONSTRAINT NoticiaSentimiento_pkey PRIMARY KEY (IdNoticia),
  CONSTRAINT NoticiaSentimiento_IdEmpresa_fkey FOREIGN KEY (IdEmpresa) REFERENCES public.Empresa(IdEmpresa)
);